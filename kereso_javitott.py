...
def keres(katalogus, szerzo=None, cim=None, kiadas=None, case_sensitive_search=True):
    ...
    for elem in katalogus:
        if "katalogus" in elem:
            ...
            if not case_sensitive_search:
                elem_szerzo = elem_szerzo.lower()
                elem_cim = elem_cim.lower()


            if (cim is None or cim in elem_cim) \
               and (szerzo is None or szerzo in elem_szerzo) \
               and (kiadas is None or
                    (isinstance(kiadas, int) and kiadas == elem["kiadas"]) or
